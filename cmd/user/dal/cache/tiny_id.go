...
func IsIDInfoExist(ctx context.Context) (exist int64, err error) {
	exist, err = RedisClient.Exists(ctx, strconv.FormatInt(constants.UserBizType, 10)).Result()
	klog.Debugf("redis biz type %v id info exist status: %v\n", constants.UserBizType, exist)
	return
}
...
func SetID(ctx context.Context, tinyID *TinyID) {
	...
	err = RedisClient.Set(ctx, strconv.FormatInt(constants.UserBizType, 10), idJson, 0).Err()
	if err != nil {
		klog.Error(err)
	}
	klog.Debugf("redis biz type %v set id info %+v\n", constants.UserBizType, tinyID)
}

func GetID(ctx context.Context) (tinyID *TinyID, err error) {
	data, err := RedisClient.Get(ctx, strconv.FormatInt(constants.UserBizType, 10)).Result()
	if err != nil {
		return nil, err
	}
